...
const runCheck = async () => {
  ...
  for (const entry of blobs) {
    ...
    // Vira o dia: zera quais refeições já foram avisadas hoje, E também zera
    // "o que já foi marcado como feito" — isso é essencial para o aviso
    // continuar funcionando mesmo que a família NUNCA mais abra o app depois
    // de configurar uma vez. Sem isso, o servidor ficaria achando pra sempre
    // que a refeição/suplemento de ontem "já foi feito", e nunca avisaria de novo.
    if (data.lastNotifyDate !== todayStr) {
      data.notifiedMealsToday = [];
      data.mealsDoneCount = 0;
